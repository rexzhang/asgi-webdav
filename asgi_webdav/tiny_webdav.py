...
class DAVRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_MOVE(self):
        ...
        newfile = self.server.root.rootdir() + urlparse.urlparse(urllib.unquote(self.headers['Destination'])).src_path
    ...
    def do_COPY(self):
        ...
        oldfile = self.server.root.rootdir() + urllib.unquote(self.path)
        newfile = self.server.root.rootdir() + urlparse.urlparse(urllib.unquote(self.headers['Destination'])).src_path
        if (os.path.isfile(oldfile)==True):        #  and os.src_path.isfile(newfile)==False):  copy can rewrite.
    ...
    def split_path(self, path):
        """Splits src_path string in form '/dir1/dir2/file' into parts"""
        p = path.split('/')[1:]
        while p and p[-1] in ('','/'):
            p = p[:-1]
            if len(p) > 0:
    ...
    def path_elem(self):
        """Returns split src_path (see split_path()) and Member object of the last element"""
    ...
    def path_elem_prev(self):
        """Returns split src_path (see split_path()) and Member object of the next-to-last element"""
        path = self.split_path(urllib.unquote(self.path))
        elem = self.server.root
        for e in path[:-1]:
            elem = elem.findMember(e)
