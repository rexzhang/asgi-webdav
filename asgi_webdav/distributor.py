...
class DAVDistributor:
    def __init__(self):
        config_path_map = [
            ...
            # ('/litmus/', '/Users/rex/p/asgi-webdav/temp/litmus'),
            ('/joplin_ext/', '/Users/rex/p/asgi-webdav/temp/ext1'),
        ]
    ...
    async def distribute(self, request: DAVRequest):
        ...
        path = request.src_path[weight:]  # TODO

        if request.method == DAV_METHOD.HEAD:
            await self.do_head(request.send, prefix, path, provider)

        elif request.method == DAV_METHOD.PROPFIND:
            await self.do_propfind(request.send, prefix, path, provider)

        elif request.method == DAV_METHOD.DELETE:
            await self.do_delete(request.send, prefix, path, provider)

        elif request.method == DAV_METHOD.MKCOL:
            await self.do_mkcol(request, prefix, path, provider)

        elif request.method == DAV_METHOD.PUT:
            await self.do_put(request, prefix, path, provider)

        elif request.method == DAV_METHOD.GET:
            await self.do_get(request, prefix, path, provider)
            ...
            await self.get_options(request.send, prefix, path, provider)

    async def get_options(
        self, send, prefix: str, path: str, provider: DAVProvider
    ):  # TODO!!!
        headers = [
            (
                b'Allow',
        ...
        await send_response_in_one_call(send, 200, headers=headers)

    async def do_head(
        self, send, prefix: str, path: str, provider: DAVProvider
    ):
        if await provider.do_head(path):
            await send_response_in_one_call(send, 200)
        else:
            await send_response_in_one_call(send, 404)
        return
    ...
    async def do_propfind(
        ...
        data = await provider.do_propfind(path)
        if data is None:
            await send_response_in_one_call(send, 404)
            return
...
9.7 DELETE
   ...
   treated as stale. Responses to this method are not cacheable.
        """
        http_status = await provider.do_delete(path)
        await send_response_in_one_call(send, http_status)
        return

    async def do_mkcol(
        self, request, prefix: str, path: str, provider: DAVProvider
    ):
        """
...
9.3.1.  MKCOL Status Codes
   ...
   method.
        ...
        http_status = await provider.do_mkcol(path)
        await send_response_in_one_call(request.send, http_status)
        return

    async def do_put(
        self, request: DAVRequest, prefix: str, path: str,
...
9.7.2.  PUT for Collections
   ...
   The MKCOL method is defined to create collections.
        ...
        http_status = await provider.do_put(path, request.receive)
        await send_response_in_one_call(request.send, http_status)
        return
...
9.3 GET
   ...
   See section 15.1.3 for security considerations when used for forms.
        """
        http_status = await provider.do_get(path, request.send)
        if http_status != 200:
            # TODO bug
            await send_response_in_one_call(request.send, http_status)
        return
